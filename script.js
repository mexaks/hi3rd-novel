...
n=0;l=0;k=0;let interval;let timeOut;var autoPlayStatus=false;let timeoutWaitingSkip;let timeoutCloseSkip;let timeoutLoad;var showSkipClaim=true;
...
$(document).on('click','.display__TextBox.working',function(){
    ...
    AddToHistory(k,n);
    if(autoPlayStatus) timeOut=setTimeout(changeText,2000)
})
...
function changeText(){ 
    ...
    if(Strings[StringsKeys[k]][n] !== undefined){
        ...
        var startType=new Date().valueOf();var endType;l=0;
...
function setCharacter(character,active=true){
    var src;
    switch(character){
        case 'Kiana':{
            src='img/Kiana.webp';
...
function setSkipClaim(){
    ...
    var dialog=document.getElementsByClassName('skipClaim');
    dialog[0].showModal();
}
function RemoveSkipClaim(){
    $('.wrapper').removeClass('lock');
    $('.skipClaim').remove();
}
function AddToHistory(k,n,name=''){
...
$('.menu__AutoPlay').click(function(){
    ...
    if(interval<=0 && !autoPlayStatus) clearTimeout(timeOut);
})
$('.menu__history').click(function(){
    $('.wrapper').toggleClass('lock');
    $('.history').toggleClass('hidden');
    var history=document.getElementsByClassName('history__content');
    history[0].scrollTop=history[0].scrollHeight;
})
$('.history__exitBtn').click(function(){
    $('.wrapper').toggleClass('lock'); 
    $('.history').toggleClass('hidden');
})